import os
import sys
import operator
import socket

os.system('chmod 777 ./*')
os.system('mkdir /root/Cylon-Raider-Lite')
os.system('mkdir /root/Cylon-Raider-Lite/Heavy-Raider')
os.system('mkdir /root/Cylon-Raider-Lite/logs')
os.system('mkdir /root/Cylon-Raider-Lite/logs/mana_toolkit')
os.system('mkdir /root/Cylon-Raider-Lite/wirelessattacks/')
os.system('mkdir /root/Cylon-Raider-Lite/')
os.system('mkdir /root/Cylon-Raider-Lite/logs/Cylon-Raider')
os.system('mkdir /root/Cylon-Raider-Lite/logs/wardriver_xpress')
os.system('mkdir /root/Cylon-Raider-Lite/logs/old')
os.system('cp -r ./* /root/Cylon-Raider-Lite/wirelessattacks/Cylon-Raider-Lite')
os.system('cp -r ./* /root/Cylon-Raider-Lite')
os.system('cp -r Cylon_Raider_Main.py /usr/local/bin')
print '[+] Install Completed, type Cylon_Raider_Main.py in terminal to get started'
