...
def thread_1():
    ...
    # run airodump-ng --band abg on the foreground
    # cmd_String = "airodump-ng --band abg wlan1mon --encrypt WPA2 --manufacturer --write %s/%s_monitor_mode_capture.cap" % (cap_file_dir, timestr)
    cmd_String = """airodump-ng --band abg wlan1mon --encrypt WPA2 --manufacturer -a --write {0}/{1}_monitor_mode_capture.csv --write-interval 5 --output-format csv""".format(
        ...
        str(timestr))
    print cmd_String
    os.system(cmd_String)# we shouldopen this in a new box to avoid disrupting the process
